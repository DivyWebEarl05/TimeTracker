...
const verifyOtp = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            user_exists: !!(user.full_name && user.email && user.state && user.city && user.work_place_name && user.position)
        });
    } catch (error) {
        res.status(500).json({ error: "Something went wrong" });
    }
...
const login = async (req, res) => {
    try {
        ...
        if (user.full_name && user.email && user.state && user.city && user.work_place_name && user.position) {
